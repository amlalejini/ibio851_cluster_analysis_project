...
####Coding From the Can
Let's generate some data to cluster hierarchically. To keep things simple, we'll stick to the data we generated for our K-means example.
```{r}
hclust.data <- kmeans.data#rnorm(100, floor(runif(5, 1, 100)), 10)
```

Okay, to make things easier, let's pre-calculate the full pair-wise dissimilarity for all of our data using the built-in dist function.

```{r}

dissim.mat <- dist(hclust.data)
```
